...
if [ "large" = "$SIZE" ]; then
    bash scripts/update-thing.sh lab9  configs/things/$THING_CONF_DIR/bj-cycle.json 9 40 no_pull &
    bash scripts/update-thing.sh hbnj2 configs/things/$THING_CONF_DIR/nj-cycle.json 2 8  no_pull &
    bash scripts/update-thing.sh hbnj5 configs/things/$THING_CONF_DIR/nj-cycle.json 5 16 no_pull &
elif [ "small" = "$SIZE" ]; then
    bash scripts/update-thing.sh lab9 configs/things/$THING_CONF_DIR/bj-cycle.json  9 2 no_pull &
    bash scripts/update-thing.sh hbnj5 configs/things/$THING_CONF_DIR/nj-cycle.json 5 2 no_pull &
fi

# bash scripts/update-thing.sh hbnj1 configs/things/$THING_CONF_DIR/nj-cycle.json 1 40 no_pull &
# bash scripts/update-thing.sh hbnj2 configs/things/$THING_CONF_DIR/nj-cycle.json 2 40 no_pull &
# bash scripts/update-thing.sh hbnj4 configs/things/$THING_CONF_DIR/nj-cycle.json 4 40 no_pull &
...
if [[ "spb" == $ENV ]] || [[ "all" == $ENV ]]; then
    python3 ./src/mock_wang.py spb $TEST_TIME_LENGTH
    echo "sleep 90s"
    sleep $((90))
fi
if [[ "net" == $ENV ]] || [[ "all" == $ENV ]]; then
    python3 ./src/mock_wang.py net $TEST_TIME_LENGTH
    echo "sleep 90s"
    sleep $((90))
fi
